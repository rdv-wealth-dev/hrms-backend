...
async function seedPermissions() {
    try {
        const operations = exports.ALL_PERMISSIONS.map((perm) => ({
            updateOne: {
        ...
        const result = await permission_model_1.PermissionModel.bulkWrite(operations);
        logger_1.logger.info({
            message: "Permissions seeded",
            inserted: result.upsertedCount,
            existing: exports.ALL_PERMISSIONS.length - result.upsertedCount,
        });
    }
    catch (error) {
        logger_1.logger.error({ message: "Permission seeding failed", error });
        throw error;
    }
}
